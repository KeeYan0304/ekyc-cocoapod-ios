...
class CropperViewController: UIViewController, Rotatable, StateRestorable, Flipable {
     let originalImage: UIImage
    var initialState: CropperState?
    var isCircular: Bool

     init(originalImage: UIImage, initialState: CropperState? = nil, isCircular: Bool = false) {
     ...
     weak var delegate: CropperViewControllerDelegate?

    // if self not init with a state, return false
     var isCurrentlyInInitialState: Bool {
    ...
    public var aspectRatioLocked: Bool = false {
     ...
     var cropContentInset: UIEdgeInsets = UIEdgeInsets(top: 20, left: 20, bottom: 20, right: 20)
